package com.evideo.nfsprovider.httpd;


import android.text.TextUtils;
import android.util.Log;

import com.evideo.nfsprovider.nativefacade.SmbFile;
...
/**
 * make sure <p/>
 *  fi.iki.elonen.NanoHTTPD.Response#sendAsFixedLength(OutputStream, int) }<p/>
 * is protected
 */
public class NanoStreamer extends NanoHTTPD implements IStreamer {
    ...
    private Response respond(Map<String, String> headers, String uri) {
        ...
        String mimeTypeForFile = NfsFileUtils.getVideoMimeType(uri);
//        String smbUri = SambaUtil.cropStreamSmbURL(uri);
        Response response = null;
        try {
            if (NfsFileUtils.isNfsUrl(uri) && !TextUtils.isEmpty(mimeTypeForFile)) {
                SmbFile smbFile = new SmbFile(uri);
                InputStream copyStream = new BufferedInputStream(new SmbFileInputStream(smbFile));
                response = serveSmbFile(uri, headers, copyStream, smbFile, mimeTypeForFile);
            } else {
